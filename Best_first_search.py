...
"""
그냥 단순 Hill-clibing 알고리즘하고 뭐가 다를까?
open리스트를 안쓴 보통 언덕 등반 알고리즘도 자식노드(자식 상태) 중 가장 높은 평가함수 가치를 가진
노드를 골랐다.

Best-firtst-search의 핵심은 이전 상태들도(이전 부모, 부모의 형제, 내 형제)
평가대상에 들어가 탐색범위가 넓어진다는 것이다.
즉 이전 상태들도 평가대상에 들어가기 때문에 지역 최적해에 빠질 가능성이 줄어든다.
좁은 지역이 문제라면 지역을 넓혀주는 것이다.

이 방식은 A* 알고리즘과 유사하다. A*알고리즘은 Best-first-search와 달리 평가함수값과
지금까지의 경로의 길이를 더한 값을 이용해 최적해를 찾는다.
하지만 Best-first-search는 경로의 길이를 고려하지 않고 평가함수값만을 이용해 최적해를 찾는다.
때문에 현문제에서 빠르게 문제를 해결한것은 운에 달려있던 것이다.

A*알고리즘은 운이 좋은 best-first-search 보다 느릴 수 있지만 문제가 보다 복잡해지면 A*알고리즘이
더 빠르게 문제를 해결할 수 있다. 그 이유는 경로의 길이를 고려해 최적해를 찾기 때문이다.
"""